...
contains
  ...
  subroutine DQMC_StSeqB_Init(n, L, nOrth, nmeas, B, SB, WS)
    !
    ! Purpose
    ! =======
    ...
    ! B matrix
    allocate(SB%U(n,n,nst))
    allocate(SB%T(n,n,nst))
    allocate(SB%D(n,nst))
    
  end subroutine DQMC_StSeqB_Init

  !----------------------------------------------------------------------!

  subroutine DQMC_SeqMultB(il, ic, SB, V)
    !
    ! Purpose
    ! =======
    ...
    ! Let U be B_{i1}
    j = il
    call DQMC_GetB(n, U, SB%B, V(:,j), SB%W1)
    ...
    do i = 2, is 
       ...
       call DQMC_MultB_Left(n, U, SB%B, V(:,j), SB%W1)
    ...
    do i = 1, is
       ...
       call DQMC_MultB_Right(n, U, SB%B, V(:,j), SB%W1)
